...
优化内容：
1. ✅ 基于真实GDP数值判断高低GDP地区（而非简单关键词匹配）
2. ✅ 使用相对误差阈值（基于流量等级动态调整）
3. ✅ 精细化天气影响分级处理（晴/多云/阴/小雨/中雨/大雨/雪等）
4. ✅ 细化事件影响分类（节假日/事故/施工/活动，区分对客货车的影响）
5. ✅ 改进样本抽取策略（避免过拟合，增加多样性）
6. ✅ 添加数据质量检查和异常值过滤
"""
...
# ============================== 日志 ==============================
class Logger(object):
    def __init__(self, filename=os.path.join(PROJECT_ROOT, f"quick518_optimized_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")):
...
print("=" * 100)
print("🚀 启动 quick518_optimized 微调 - 全面优化版")
print("✅ 基于真实GDP、相对误差阈值、精细化天气/事件判断")
print("=" * 100)
...
DATASET_CACHE_PATH = os.path.join(PROJECT_ROOT, "quick518_optimized.json")
RESULTS_DIR = os.path.join(PROJECT_ROOT, "results-quick518-optimized")
FINAL_MODEL_DIR = os.path.join(PROJECT_ROOT, "llama-3-1-8b-highway-finetuned-quick518-optimized")
...
NORMAL_SAMPLE_RATIO = 0.15  # 略微提高正常样本比例，增加多样性
...
# ============================== 【优化1】基于真实GDP数值判断高低GDP地区 ==============================
def parse_gdp_from_description(desc):
    """从站点描述中提取真实GDP数值（单位：亿元）"""
    # 匹配模式：GDP is XXX.XX billion yuan
    pattern = r'GDP\s+is\s+([\d,]+\.?\d*)\s+billion\s+yuan'
    match = re.search(pattern, desc, re.IGNORECASE)
    
    if match:
        gdp_str = match.group(1).replace(',', '')
        try:
...
def infer_gdp_level_with_threshold(desc, gdp_threshold=1000.0):
    ...
    参数:
    - desc: 站点描述文本
    - gdp_threshold: GDP阈值（亿元），默认1000亿
...
print("🔍 GDP解析测试:")
print("=" * 70)
test_stations = [0, 1, 10, 50, 97]
for idx in test_stations:
    if idx < len(carpark_des_list):
        desc = carpark_des_list[idx]
        gdp = parse_gdp_from_description(desc)
        level = infer_gdp_level_with_threshold(desc)
        s_name = desc.split()[0]
        print(f"   站点{idx:3d} [{s_name:15s}]: GDP={gdp if gdp else 'N/A':>10} 亿元, 等级={level}")
print("=" * 70)
...
def calculate_dynamic_threshold(true_values, feature_idx, base_threshold_pct=0.15):
    ...
    原理:
    - 高流量场景（如峰值1000+）: 允许较大绝对误差（15%相对误差）
    - 低流量场景（如夜间50以下）: 要求较小绝对误差（但最小不低于固定阈值）
    
    参数:
    - true_values: 真实值数组 (8小时序列)
    - feature_idx: 特征索引 (0-3)
    - base_threshold_pct: 基础相对误差百分比（默认15%）
    ...
    # 不同特征的基准阈值（考虑量级差异）
    base_thresholds = {
        0: 80,   # 小客车上行：峰值可达1600，阈值80
        1: 80,   # 小客车下行：峰值可达1400，阈值80
        2: 40,   # 非小客车上行：峰值约500，阈值40
        3: 40,   # 非小客车下行：峰值约500，阈值40
    }
...
# 预计算每个特征的全局统计信息，用于动态阈值
print("\n" + "=" * 70)
print("📊 各特征流量统计（用于动态阈值计算）:")
print("=" * 70)
feature_stats = {}
for f in range(num_features):
    all_true_f = true_array[:, :, f, :].flatten()
    feature_stats[f] = {
        'mean': np.mean(all_true_f),
        'std': np.std(all_true_f),
        'min': np.min(all_true_f),
        'max': np.max(all_true_f),
        'median': np.median(all_true_f),
        'q75': np.percentile(all_true_f, 75),
    }
    f_name = FEATURE_NAMES[f]
    stats = feature_stats[f]
    print(f"   {f_name:25s}: 均值={stats['mean']:7.1f}, 中位数={stats['median']:7.1f}, "
          f"75分位={stats['q75']:7.1f}, 最大值={stats['max']:7.1f}")
print("=" * 70 + "\n")
...
WEATHER_SEVERITY_MAP = {
    # 无影响
    "晴": {"severity": 0, "impact": "none", "desc": "Clear"},
    "多云": {"severity": 0, "impact": "none", "desc": "Cloudy"},
    "阴": {"severity": 1, "impact": "slight", "desc": "Overcast"},
    
    # 轻度影响
    "小雨": {"severity": 2, "impact": "moderate_decrease", "desc": "Light Rain"},
    "阵雨": {"severity": 2, "impact": "moderate_decrease", "desc": "Shower"},
    "雾": {"severity": 2, "impact": "moderate_decrease", "desc": "Fog"},
    "霾": {"severity": 2, "impact": "moderate_decrease", "desc": "Haze"},
    
    # 中度影响
    "中雨": {"severity": 3, "impact": "significant_decrease", "desc": "Moderate Rain"},
    "大雨": {"severity": 4, "impact": "severe_decrease", "desc": "Heavy Rain"},
    "暴雨": {"severity": 5, "impact": "severe_decrease", "desc": "Rainstorm"},
    "小雪": {"severity": 3, "impact": "significant_decrease", "desc": "Light Snow"},
    
    # 严重影响
    "中雪": {"severity": 4, "impact": "severe_decrease", "desc": "Moderate Snow"},
    "大雪": {"severity": 5, "impact": "severe_decrease", "desc": "Heavy Snow"},
    "暴雪": {"severity": 5, "impact": "severe_decrease", "desc": "Blizzard"},
    "冰雹": {"severity": 5, "impact": "severe_decrease", "desc": "Hail"},
}


def get_weather_impact(weather_str):
    """
    获取天气影响的详细信息
    
    返回:
    - severity: 严重程度 (0-5)
    - impact: 影响类型 ("none", "slight", "moderate_decrease", "significant_decrease", "severe_decrease")
    - desc: 英文描述
    """
    if not weather_str or weather_str == "Unknown":
        return {"severity": 0, "impact": "unknown", "desc": "Unknown"}
    
    # 精确匹配
    if weather_str in WEATHER_SEVERITY_MAP:
        return WEATHER_SEVERITY_MAP[weather_str]
    
    # 模糊匹配
    weather_lower = str(weather_str).lower()
    
    if any(kw in weather_lower for kw in ["晴", "clear", "sunny"]):
        return WEATHER_SEVERITY_MAP["晴"]
    elif any(kw in weather_lower for kw in ["多云", "cloudy"]):
        return WEATHER_SEVERITY_MAP["多云"]
    elif any(kw in weather_lower for kw in ["阴", "overcast"]):
        return WEATHER_SEVERITY_MAP["阴"]
    elif any(kw in weather_lower for kw in ["小雨", "light rain", "drizzle"]):
        return WEATHER_SEVERITY_MAP["小雨"]
    elif any(kw in weather_lower for kw in ["中雨", "moderate rain"]):
        return WEATHER_SEVERITY_MAP["中雨"]
    elif any(kw in weather_lower for kw in ["大雨", "heavy rain", "暴雨", "rainstorm"]):
        return WEATHER_SEVERITY_MAP["大雨"]
    elif any(kw in weather_lower for kw in ["小雪", "light snow"]):
        return WEATHER_SEVERITY_MAP["小雪"]
    elif any(kw in weather_lower for kw in ["中雪", "moderate snow"]):
        return WEATHER_SEVERITY_MAP["中雪"]
    elif any(kw in weather_lower for kw in ["大雪", "heavy snow", "暴雪", "blizzard"]):
        return WEATHER_SEVERITY_MAP["大雪"]
    elif any(kw in weather_lower for kw in ["雾", "fog", "mist"]):
        return WEATHER_SEVERITY_MAP["雾"]
    elif any(kw in weather_lower for kw in ["霾", "haze", "smog"]):
        return WEATHER_SEVERITY_MAP["霾"]
    
    return {"severity": 0, "impact": "unknown", "desc": weather_str}


# ============================== 【优化4】细化事件影响分类 ==============================
EVENT_IMPACT_MAP = {
    # 节假日类（对小客车↑，对货车↓）
    "Mid-Autumn Festival": {"type": "holiday", "passenger_impact": "increase", "freight_impact": "decrease"},
    "National Day": {"type": "holiday", "passenger_impact": "increase", "freight_impact": "decrease"},
    "Spring Festival": {"type": "holiday", "passenger_impact": "increase", "freight_impact": "decrease"},
    "Qingming Festival": {"type": "holiday", "passenger_impact": "increase", "freight_impact": "decrease"},
    "Dragon Boat Festival": {"type": "holiday", "passenger_impact": "increase", "freight_impact": "decrease"},
    
    # 事故/管制类（对所有车型↓）
    "accident": {"type": "incident", "passenger_impact": "decrease", "freight_impact": "decrease"},
    "construction": {"type": "incident", "passenger_impact": "decrease", "freight_impact": "decrease"},
    "congestion": {"type": "incident", "passenger_impact": "decrease", "freight_impact": "decrease"},
    "control": {"type": "incident", "passenger_impact": "decrease", "freight_impact": "decrease"},
    "closed": {"type": "incident", "passenger_impact": "decrease", "freight_impact": "decrease"},
    "road closed": {"type": "incident", "passenger_impact": "decrease", "freight_impact": "decrease"},
    "blocked": {"type": "incident", "passenger_impact": "decrease", "freight_impact": "decrease"},
    
    # 活动类（对小客车↑，对货车影响不定）
    "concert": {"type": "event", "passenger_impact": "increase", "freight_impact": "fluctuate"},
    "match": {"type": "event", "passenger_impact": "increase", "freight_impact": "fluctuate"},
    "game": {"type": "event", "passenger_impact": "increase", "freight_impact": "fluctuate"},
    "activity": {"type": "event", "passenger_impact": "increase", "freight_impact": "fluctuate"},
    "festival": {"type": "event", "passenger_impact": "increase", "freight_impact": "fluctuate"},
    "exhibition": {"type": "event", "passenger_impact": "increase", "freight_impact": "fluctuate"},
}
...
def infer_detailed_event_impact(ev_text, is_passenger_car=True):
    ...
    参数:
    - ev_text: 事件描述文本
    - is_passenger_car: 是否为小客车特征
    
    返回:
    - impact_type: 影响类型 ("increase", "decrease", "fluctuate", "none")
    - impact_magnitude: 影响程度 ("slight", "moderate", "significant", "severe")
    - event_category: 事件类别 ("holiday", "incident", "event", "none")
    """
    ...
    # 先尝试精确匹配
    for keyword, impact_info in EVENT_IMPACT_MAP.items():
        if keyword.lower() in ev_lower:
            impact_key = "passenger_impact" if is_passenger_car else "freight_impact"
            return impact_info[impact_key], "significant", impact_info["type"]
    
    # 模糊匹配
    holiday_keywords = ["festival", "holiday", "national day", "spring festival", "mid-autumn"]
    incident_keywords = ["accident", "construction", "congestion", "control", "closed", "blocked", "delay"]
    event_keywords = ["concert", "match", "game", "activity", "exhibition", "show"]
    
    if any(kw in ev_lower for kw in holiday_keywords):
        impact = "increase" if is_passenger_car else "decrease"
        return impact, "significant", "holiday"
    elif any(kw in ev_lower for kw in incident_keywords):
        return "decrease", "severe", "incident"
    elif any(kw in ev_lower for kw in event_keywords):
        return "increase" if is_passenger_car else "fluctuate", "moderate", "event"
    
    return "fluctuate", "moderate", "unknown"
...
def load_chinese_events(path):
    ...
    try:
        ...
        for encoding in ['utf-8', 'gb18030', 'gbk']:
            ...
            raise ValueError("无法读取事件文件，编码不兼容")
...
def process_chunk(stations_chunk):
    ...
    for j in stations_chunk:
        ...
        # 【优化1】基于真实GDP判断
        gdp_value = parse_gdp_from_description(desc)
        gdp_level = infer_gdp_level_with_threshold(desc)
        ...
        for f in range(num_features):
            ...
            # 【优化2】使用动态相对误差阈值
            need_correction = np.zeros(len(mae_per_sample), dtype=bool)
            for i in range(len(mae_per_sample)):
                dynamic_thresh = calculate_dynamic_threshold(true_j[i], f)
                need_correction[i] = mae_per_sample[i] > dynamic_thresh

            # 事件触发：所有有事件的样本都纳入
            ev_mask = has_ev
            
            # 【优化5】分层随机采样：确保覆盖不同流量区间
            norm_mask = np.zeros(len(mae_per_sample), dtype=bool)
            ...
            norm_mask |= (mid_flow_mask & (np.random.rand(len(mae_per_sample)) < NORMAL_SAMPLE_RATIO))
            ...
            for i in keep:
                try:
                    ...
                    # 判断是否超过动态阈值
                    dynamic_thresh = calculate_dynamic_threshold(true_value_seq, f)
                    is_above_threshold = mae_per_sample[i] > dynamic_thresh
                    ...
                    if gdp_value:
                        cot.append(
                            ...
                            f"(GDP: {gdp_value:.1f} billion yuan). "
                            f"This determines the base sensitivity to holiday and economic activities."
                        )
                    else:
                        cot.append(
                            f"1. Economic Profile: Station {s_name} is in a {gdp_level}-GDP region. "
                            f"This affects baseline traffic patterns."
                        )
                    ...
                    if weather_info["severity"] == 0:
                        cot.append(
                            f"4. Weather Impact: Favorable weather ({w}) causes no disruption to baseline capacity."
                        )
                    elif weather_info["severity"] <= 2:
                        cot.append(
                            f"4. Weather Impact: Mild adverse weather ({w}) may cause slight traffic reduction, "
                            f"particularly during {time_period}."
                        )
                    elif weather_info["severity"] <= 3:
                        cot.append(
                            f"4. Weather Impact: Moderate adverse weather ({w}) restricts road capacity, "
                            f"causing noticeable drop in traffic volume during {time_period}."
                        )
                    else:
                        cot.append(
                            f"4. Weather Impact: Severe adverse weather ({w}) significantly restricts road capacity, "
                            f"causing substantial drop in traffic volume, especially critical during {time_period}."
                        )

                    # 【优化4】精细化事件影响
                    if ev != "None":
                        impact_type, impact_mag, event_cat = infer_detailed_event_impact(ev, is_p)
                        
                        if impact_type == "decrease":
                            if impact_mag == "severe":
                                cot.append(
                                    f"5. Event Disruption: A severe negative event ('{ev}') drastically throttles traffic flow downwards."
                                )
                            else:
                                cot.append(
                                    f"5. Event Disruption: A negative event ('{ev}') reduces typical traffic flow."
                                )
                        elif impact_type == "increase":
                            if impact_mag == "significant":
                                cot.append(
                                    f"5. Event Disruption: A major positive event ('{ev}') draws substantial localized traffic, pushing volume upwards."
                                )
                            else:
                                cot.append(
                                    f"5. Event Disruption: A positive event ('{ev}') moderately increases local traffic."
                                )
                        else:
                            cot.append(
                                f"5. Event Disruption: Special event ('{ev}') causes anomalous fluctuations in traffic patterns."
                            )
                    else:
                    ...
                    if is_above_threshold:
                        ...
                        cot.append(
                            "6. Strategy: Standard GNN captures this routine variance well. Minor adjustment based on contextual factors."
                        )
...
if comparison_log:
    ...
    comparison_df.to_csv(
        os.path.join(PROJECT_ROOT, "comparison_log_quick518_optimized.csv"),
        index=False,
        encoding="utf-8-sig"
    )
...
if generated_dataset:
    ...
    if comparison_log:
        ...
        final_comparison_df.to_csv(
            os.path.join(PROJECT_ROOT, "final_comparison_log_quick518_optimized.csv"),
            index=False,
            encoding="utf-8-sig"
        )

...
# ==============================
# 批量生成样本（【核心】只有误差超过阈值才触发修正）
# ==============================
NORMAL_SAMPLE_RATIO = 0.1  # 正常样本采样比例（用于保持多样性，但不触发修正）

def process_chunk(stations_chunk):
    """处理站点块 - 【核心】只有误差超过特征特定阈值才触发修正"""
    samples = []
            ...
            for i in keep:
                try:
                    ...
                    if is_above_threshold:
                        ...
                        mae_after = np.abs(np.array(cor) - true_value_seq).mean()
                        
                        # 记录对比日志
                        t3_str = t3.strftime('%Y-%m-%d %H:%M:%S')
                        log_comparison(s_name, f_name, t3_str, 
                                     gnn_array, true_value_seq.tolist(), cor,
                                     mae_before, mae_after)
                        
                        # 构建CoT文本（使用修正后的值）
                        final_values = cor
                    else:
                        # 误差未超过阈值：不触发修正，直接使用GNN原始值
                    ...
                    if is_above_threshold:
                        ...
                        if is_p:
                            ...
                            cot.append(f"3. Reasoning: Truck flow is stable; only reduced on holidays.")
                        ...
                        # 误差未超过阈值：简化推理
                        cot.append(f"3. Reasoning: GNN prediction is accurate enough (MAE ≤ {feature_threshold}), no correction needed.")
                        cot.append(f"Final Correction: [{', '.join(map(str, final_values))}]")
...
if generated_dataset:
    ...
    # ========== 【断点恢复】查找最新的checkpoint ==========
    checkpoint_dir = os.path.join(PROJECT_ROOT, "results-quick")
    ...
    if comparison_log:
        ...
        print(f"   平均改善率: {100 * final_comparison_df['improvement'].mean() / final_comparison_df['gnn_error'].mean():.1f}%")

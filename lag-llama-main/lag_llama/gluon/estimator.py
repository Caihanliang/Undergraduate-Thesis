...
class LagLlamaEstimator(PyTorchLightningEstimator):
    ...
    def __init__(
        ...
        # 关键修复: 检测lags_seq是整数列表还是频率字符串列表
        if len(lags_seq) > 0 and isinstance(lags_seq[0], int):
            # 已经是整数索引列表,直接使用(预训练模型的情况)
            self.lags_seq = sorted(set(lags_seq))
            print(f"[Estimator] 使用整数lags_seq, 长度: {len(self.lags_seq)}")
        else:
            # 频率字符串列表,需要转换(标准GluonTS情况)
            ...
            if len(lag_indices):
                ...
                self.lags_seq = []
            print(f"[Estimator] 从频率字符串转换lags_seq, 长度: {len(self.lags_seq)}")

...
class FaSTVisualizationRunner:
    ...
    def _setup_components(self):
        ...
        # # 原来的 dataset 内部会找不到路径，我们直接绕过内部加载，自己传正确路径
        # data_root = os.path.join("main-master", "datasets", dataset_cfg.dataset_name)
        # sample_path = f"{input_len}_{output_len}"
        # full_sample_path = os.path.join(data_root, sample_path)

        # self.logger.info(f"✅ 加载索引文件夹：{full_sample_path}")
        # # 手动加载索引，不让 dataset 内部乱找
        # self.idx_train = np.load(os.path.join(full_sample_path, "idx_train.npy"))
        # self.idx_val   = np.load(os.path.join(full_sample_path, "idx_val.npy"))
        # self.idx_test  = np.load(os.path.join(full_sample_path, "idx_test.npy"))
    ...
    # ====================== 【最终修复：CSV 时间 100% 对齐】 ======================
    def save_all_nodes_csv(self, pred, target):
        pred_step0 = pred[:, 0, :, 0]  # 仅取第一个时间步保留下结果
        true_step0 = target[:, 0, :, 0]
        pred_real = pred_step0
        true_real = true_step0
...
python "可视化测试161LC.py" --mode test -c main-master/FaST/HNGS_24_8LC.py     -ckpt main-master/checkpoints/FaST/HNGS_LC_50_24_8/727bd5e506f06d14ac9f52e57f203ab1/FaST_best_val_MAE.pt     -g 0
/home/user/Downloads/cai/FaST-main-24_8/FaST-main/main-master/checkpoints/FaST/HNGS_LC_50_24_8/727bd5e506f06d14ac9f52e57f203ab1/FaST_best_val_MAE.pt

python "可视化测试161LC.py" --mode test -c main-master/FaST/HNGS_8_8LC.py     -ckpt main-master/checkpoints/FaST/HNGS_LC_50_8_8/7bcb3a0ef45f2a47d45246b0ea0d461c/FaST_best_val_MAE.pt     -g 0
/home/user/Downloads/cai/FaST-main-24_8/FaST-main/main-master/checkpoints/FaST/HNGS_LC_50_8_8/7bcb3a0ef45f2a47d45246b0ea0d461c/FaST_best_val_MAE.pt

python "可视化测试161LC.py" --mode train -c main-master/FaST/HNGS_24_24LC.py     -ckpt main-master/checkpoints/FaST/HNGS_LC_50_8_24/2962599dcbdecb73b76508beea2fcda6/FaST_best_val_MAE.pt     -g 0

checkpoints/FaST/HNGS_LC_50_8_24/2962599dcbdecb73b76508beea2fcda6/FaST_48.pt
checkpoints/FaST/HNGS_LC_50_8_24/2962599dcbdecb73b76508beea2fcda6/FaST_best_val_MAE.pt

...
class MyTimeSeries(BaseDataset):
    ...
    def _load_data(self) -> np.ndarray:
        ...
        # 尝试多个可能的路径
        possible_paths = [
            self.data_file_path,  # datasets/HNGS_4FEAT
            f"main-master/{self.data_file_path}",  # main-master/datasets/HNGS_4FEAT
            os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), self.data_file_path),
        ]
        ...
        for base_path in possible_paths:
            ...
            if os.path.exists(his_file):
                if self.logger:
                    self.logger.info(f"Loading data from: {his_file}")
                else:
                    print(f"[INFO] Loading data from: {his_file}")
        ...
        if not data_loaded:
            raise FileNotFoundError(
                f"Cannot find his.npz in any of the following paths:\n" +
                "\n".join([p + "/his.npz" for p in possible_paths])
            )
        
        # 加载预划分的索引
        idx_file_found = False
        for base_path in possible_paths:
            train_idx_file = base_path + sample_path + "/idx_train.npy"
            if os.path.exists(train_idx_file):
                if self.logger:
                    self.logger.info(f"Loading indices from: {base_path}{sample_path}/")
                else:
                    print(f"[INFO] Loading indices from: {base_path}{sample_path}/")
        ...
        if not idx_file_found:
            raise FileNotFoundError(
                f"Cannot find index files in any of the following paths:\n" +
                "\n".join([p + sample_path + "/idx_train.npy" for p in possible_paths])
            )
        
        total_length = len(data)
        
        # 根据 mode 截取对应数据段（添加边界保护）
        if self.mode == "train":
            # 确保起始索引不小于0
            start_idx = max(0, train_idx[0] - self.output_len - self.input_len + 1)
            # 确保结束索引不超过数据长度
            end_idx = min(total_length, train_idx[-1] + 1)
            if self.logger:
                self.logger.info(f"Train data slice: [{start_idx}:{end_idx}], length={end_idx-start_idx}, original indices [{train_idx[0]}:{train_idx[-1]}]")
            else:
                print(f"[INFO] Train data slice: [{start_idx}:{end_idx}], length={end_idx-start_idx}")
            sliced_data = data[start_idx : end_idx].copy()
        elif self.mode == "valid":
            start_idx = max(0, val_idx[0] - self.output_len - self.input_len + 1)
            end_idx = min(total_length, val_idx[-1] + 1)
            if self.logger:
                self.logger.info(f"Val data slice: [{start_idx}:{end_idx}], length={end_idx-start_idx}, original indices [{val_idx[0]}:{val_idx[-1]}]")
            else:
                print(f"[INFO] Val data slice: [{start_idx}:{end_idx}], length={end_idx-start_idx}")
            sliced_data = data[start_idx : end_idx].copy()
        else:  # test
            start_idx = max(0, test_idx[0] - self.output_len - self.input_len + 1)
            if self.logger:
                self.logger.info(f"Test data slice: [{start_idx}:], length={total_length-start_idx}, original indices [{test_idx[0]}:]")
            else:
                print(f"[INFO] Test data slice: [{start_idx}:], length={total_length-start_idx}")
            sliced_data = data[start_idx :].copy()
        
        if self.logger:
            self.logger.info(f"Sliced data shape: {sliced_data.shape}, expected samples: {len(sliced_data) - self.input_len - self.output_len + 1}")
        else:
            print(f"[INFO] Sliced data shape: {sliced_data.shape}")
        return sliced_data

...
# 不推理这些站点
# SKIP_STATION_IDS = {6, 10, 11, 12, 13, 14, 16, 22, 43}
SKIP_STATION_IDS = {5,6, 9,10, 11, 12, 13, 14, 16,17,18, 22, 43,82}


# 加速建议：先用 32；如果显存足够，可以试 48 / 64
BATCH_SIZE = 32
...
# ====================== 输出文件 ======================
# 所有推理结果统一放到这个文件夹下
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "inference_results_skip_14stations")
os.makedirs(OUTPUT_DIR, exist_ok=True)

DETAIL_PATH = os.path.join(OUTPUT_DIR, "all_station_timeslot_feature_metrics.csx")
STATION_FEATURE_MEAN_PATH = os.path.join(OUTPUT_DIR, "all_station_feature_mean_metrics.csx")
GLOBAL_FEATURE_MEAN_PATH = os.path.join(OUTPUT_DIR, "global_feature_mean_metrics.csx")
GLOBAL_OVERALL_MEAN_PATH = os.path.join(OUTPUT_DIR, "global_overall_mean_metrics.csx")
...
if DEBUG_MAX_STATIONS is None:
    station_range = [
        sid for sid in range(num_stations)
        if sid not in SKIP_STATION_IDS
    ]
else:
    station_range = [
        sid for sid in range(min(DEBUG_MAX_STATIONS, num_stations))
        if sid not in SKIP_STATION_IDS
    ]

print(f"🚫 跳过站点: {sorted(SKIP_STATION_IDS)}")
...
def generate_texts(prompts, max_new_tokens, batch_size, desc="生成中"):
    """
    加速要点：
    1. 不在每个 batch 后 torch.cuda.empty_cache()
    2. max_new_tokens 很短
    3. num_beams=1
    4. do_sample=False
    """
...
try:
    for station_id in tqdm(station_range, desc="总站点进度"):
        ...
        for feature_id in range(num_features):
            ...
            # 如果 station_id=0 不被跳过，这里仍然会打印首个站点首个特征的 debug
...
with open(GLOBAL_OVERALL_MEAN_PATH, "w", newline="", encoding="utf-8-sig") as f:
    ...
    writer.writerow([
        "ALL_STATIONS_EXCEPT_SKIP_14_ALL_FEATURES",
...
print("✅ 全站点推理完成")
print("=" * 100)
print(f"🚫 已跳过站点: {sorted(SKIP_STATION_IDS)}")

...
# 【关键修复】直接使用已有的checkpoint-780，不需要重新训练！
# MODEL_PATH = os.path.join(PROJECT_ROOT, "results-quick430/checkpoint-780") #results-quick506/checkpoint-9380
MODEL_PATH = os.path.join(PROJECT_ROOT, "results-quick506/checkpoint-9380") #results-quick506/checkpoint-9380
...
# 【调试开关】设置为True可以看到LLM原始输出
DEBUG_MODE = True
debug_sample_count = 0  # 已打印的调试样本数
MAX_DEBUG_SAMPLES = 3   # 最多打印3个样本的完整输出
    ...
    for start_idx in tqdm(range(0, total, batch_size), desc="批量推理中"):
        ...
        with torch.no_grad():
            outputs = model.generate(
                **inputs,
                max_new_tokens=128,  # 【增加】从64改为128，确保有足够空间输出
                temperature=0.01,
                top_p=0.95,
                do_sample=True,
        ...
        for i in range(len(batch_prompts)):
            ...
            if DEBUG_MODE and debug_sample_count < MAX_DEBUG_SAMPLES:
                print(f"\n{'='*80}")
                print(f"📄 LLM原始输出 (样本 {start_idx + i}):")
                print(f"{'='*80}")
                print(text[-500:] if len(text) > 500 else text)  # 打印最后500字符
                print(f"{'='*80}\n")
...
for f in range(4):
    ...
    for i in range(N_TEST):
        ...
        llm_results[f].append(res[0] if res and len(res) > 0 else gnn_seq[0])

import os
# 1. 彻底关闭 XLA 编译优化（这是报错的根源）
os.environ['TF_XLA_FLAGS'] = '--tf_xla_cpu_global_jit=false'
os.environ['TF_XLA_FLAGS'] = '' # 清空可能导致解析错误的 flag

# 2. 禁用 MLIR 优化器的另一种方式（针对 TF 2.x 兼容模式）
os.environ['TF_MLIR_OPTIMIZE_CONCRETE_FUNCTIONS'] = '0'

# 3. 限制显存增长（防止新显卡驱动与老 TF 抢夺句柄）
os.environ['TF_FORCE_GPU_ALLOW_GROWTH'] = 'true'

# 4. 如果还是报错，请暂时取消注释下面这行用 CPU 跑（34个站 CPU 很快）
...
parser.add_argument('--batch_size', type = int, default = 32,
                    help = 'batch size')
parser.add_argument('--max_epoch', type = int, default =1000,#1000
                    help = 'epoch to run')
...
parser.add_argument('--decay_epoch', type=int, default = 5,
                    help = 'decay epoch')
parser.add_argument('--traffic_file', default = '/home/user/FSTLLM/contrast/Traffic-Benchmark-master/methods/GMAN/METR/data/subway.h5',
                    help = 'traffic file')
parser.add_argument('--SE_file', default = '/home/user/FSTLLM/contrast/Traffic-Benchmark-master/methods/GMAN/METR/data/subway/SE.txt',
                    help = 'spatial emebdding file')
parser.add_argument('--model_file', default = '/home/user/FSTLLM/contrast/Traffic-Benchmark-master/methods/GMAN/data/GMAN(subway)',
                    help = 'save the model to disk')
parser.add_argument('--log_file', default = '/home/user/FSTLLM/contrast/Traffic-Benchmark-master/methods/GMAN/data/log(subway)',
                    help = 'log file')

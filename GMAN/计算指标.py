...
# 特征列名映射（根据实际CSV文件的列名）
FEATURE_COLUMNS = {
    '小客车上行': ('小客车上行_预测值', '小客车上行_真实值'),
    '小客车下行': ('小客车下行_预测值', '小客车下行_真实值'),
    '非小客车上行': ('非小客车上行_预测值', '非小客车上行_真实值'),
    '非小客车下行': ('非小客车下行_预测值', '非小客车下行_真实值')
}
...
def calculate_metrics(y_true, y_pred):
    ...
    # 避免除以0的情况
    mask_nonzero = y_true_valid != 0
    if np.sum(mask_nonzero) > 0:
        mape = np.mean(np.abs((y_true_valid[mask_nonzero] - y_pred_valid[mask_nonzero]) / 
                              y_true_valid[mask_nonzero])) * 100
    else:
        mape = np.nan

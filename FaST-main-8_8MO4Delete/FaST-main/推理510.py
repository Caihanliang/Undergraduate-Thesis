...
# ====================== 推理范围 ======================
# None = 推理全部 157 个站点
# 只推理第0个站点：RUN_STATION_IDS = [0]
# 推理多个站点：RUN_STATION_IDS = [0, 1, 2]
# RUN_STATION_IDS = None
RUN_STATION_IDS = [10]

# 每个站点最多推理多少样本；None = 全部
DEBUG_MAX_SAMPLES_PER_STATION = None

# ====================== 推理速度配置 ======================
BATCH_SIZE = 32

# quick506 训练时是 COT + Final Correction，所以第一轮需要稍长一点
COT_MAX_NEW_TOKENS = 220

# 第二轮强制直接输出 Final Correction，短一些
FORCE_MAX_NEW_TOKENS = 64

# ====================== 策略配置 ======================
# 根据你已有结果：
# 0/1 客车特征 LLM 变差，所以默认不跑 LLM，直接用 GNN
# 2/3 非客车特征 LLM 变好，所以使用 LLM
USE_LLM_BY_FEATURE = {
    0: True,
    1: True,
    2: True,
...
# 是否启用安全门控：LLM 输出离 GNN 太远就回退 GNN
USE_SAFE_GATE = True

# 是否启用融合：final = alpha * LLM + (1-alpha) * GNN
# 作用：缓解 quick506 过度修正
USE_BLEND = True
...
# ====================== 输出文件 ======================
DETAIL_PATH = os.path.join(PROJECT_ROOT, "all_station_timeslot_feature_metrics.csx")
STATION_FEATURE_MEAN_PATH = os.path.join(PROJECT_ROOT, "all_station_feature_mean_metrics.csx")
GLOBAL_FEATURE_MEAN_PATH = os.path.join(PROJECT_ROOT, "global_feature_mean_metrics.csx")
GLOBAL_OVERALL_MEAN_PATH = os.path.join(PROJECT_ROOT, "global_overall_mean_metrics.csx")

# ====================== Prompt：必须贴近 quick506 训练格式 ======================
# quick506 训练时就是这个 instruction，assistant 后面输出 COT + Final Correction
INSTRUCTION_TEXT = (
...
def calc_metrics(y_true, y_pred, eps=1e-6):
    ...
    mae = np.mean(abs_err)
    mape = np.mean(abs_err / (y_true + eps)) * 100
...
# ====================== 后处理：安全门控 + 融合 ======================
def safe_gate_llm(llm_seq, gnn_seq, feature_id):
    """
    不使用真实值，只根据 LLM 与 GNN 的偏离判断是否可信。
    """
    gnn_seq = np.asarray(gnn_seq, dtype=np.int32)
    gnn_seq = np.maximum(gnn_seq, 0)

    if llm_seq is None:
        return None, 1, "parse_fail"

    llm_seq = np.asarray(llm_seq, dtype=np.int32)
    llm_seq = np.maximum(llm_seq, 0)

    if len(llm_seq) != len(gnn_seq):
        return None, 1, "length_error"

    # quick506 容易过修正，所以设置偏离限制
    if feature_id in [0, 1]:
        max_ratio = 0.25
        max_abs = 80
    else:
        max_ratio = 0.70
    ...
    diff = np.abs(llm_seq - gnn_seq)
    allowed = np.maximum(np.abs(gnn_seq) * max_ratio, max_abs)

    if np.any(diff > allowed):
        return None, 1, "too_far_from_gnn"

    # 检查跳变异常
    if len(llm_seq) >= 2:
        llm_jump = np.abs(np.diff(llm_seq))
        gnn_jump = np.abs(np.diff(gnn_seq))

        max_gnn_jump = max(float(np.max(gnn_jump)), 1.0)
        max_llm_jump = float(np.max(llm_jump))

        if feature_id in [0, 1]:
            jump_ratio_limit = 3.0
            jump_abs_limit = 160
        else:
            jump_ratio_limit = 4.0
            jump_abs_limit = 100

        if max_llm_jump > max(max_gnn_jump * jump_ratio_limit, jump_abs_limit):
            return None, 1, "abnormal_jump"

    return llm_seq, 0, "pass_gate"


def blend_llm_gnn(llm_seq, gnn_seq, feature_id):
    """
    final = alpha * LLM + (1 - alpha) * GNN
    """
    llm_seq = np.asarray(llm_seq, dtype=float)
    gnn_seq = np.asarray(gnn_seq, dtype=float)

    alpha = BLEND_ALPHA_BY_FEATURE.get(feature_id, 1.0)

    final_seq = alpha * llm_seq + (1.0 - alpha) * gnn_seq
    final_seq = np.rint(final_seq).astype(np.int32)
    final_seq = np.maximum(final_seq, 0)
...
if RUN_STATION_IDS is None:
    station_range = list(range(num_stations))
else:
    station_range = [int(x) for x in RUN_STATION_IDS if 0 <= int(x) < num_stations]

print(f"🚀 将推理站点数: {len(station_range)}")
print(f"🚀 推理站点 ID: {station_range[:20]}{' ...' if len(station_range) > 20 else ''}")
print(f"🚀 每站样本限制: {DEBUG_MAX_SAMPLES_PER_STATION if DEBUG_MAX_SAMPLES_PER_STATION is not None else '全部'}")
print(f"🚀 批大小: {BATCH_SIZE}")
print(f"🚀 特征 LLM 开关: {USE_LLM_BY_FEATURE}")
print(f"🚀 安全门控: {USE_SAFE_GATE}")
print(f"🚀 融合策略: {USE_BLEND}, alpha={BLEND_ALPHA_BY_FEATURE}")
...
def predict_feature_with_retry(prompt_items, expected_len, batch_size=32, print_debug=False):
    """
    quick506 专用：
    第一轮：用训练格式，让模型输出 COT + Final Correction
    第二轮：失败后强制输出 Final Correction
    """
    ...
    strategies = [
        ("第1轮 COT格式推理", "prompt_std", COT_MAX_NEW_TOKENS, False),
        ("第2轮 强制Final重试", "prompt_force", FORCE_MAX_NEW_TOKENS, True),
    ]
    ...
    for round_name, prompt_key, max_new_tokens, allow_naked in strategies:
        ...
        if print_debug:
            ...
            if new_fail_indices:
                ...
                print("\n" + "=" * 90)
                print(f"🔍 [DEBUG] 失败样本输出：")
                print("=" * 90)
                print(raw_outputs[debug_idx][-800:])
                print("=" * 90 + "\n")
...
detail_writer.writerow(detail_header)

station_feature_f = open(STATION_FEATURE_MEAN_PATH, "w", newline="", encoding="utf-8-sig")
station_feature_writer = csv.writer(station_feature_f)
station_feature_writer.writerow(station_feature_mean_header)
...
try:
    for station_id in tqdm(station_range, desc="总站点进度"):
        ...
        for feature_id in range(num_features):
            ...
            use_llm_this_feature = USE_LLM_BY_FEATURE.get(feature_id, True)

            if use_llm_this_feature:
                prompt_items = [
                    build_prompt_meta(station_id, i, feature_id, station_timestamps)
                    for i in range(usable_samples)
                ]

                print_debug = (station_id == station_range[0] and feature_id == 2)
                ...
                prompt_items = []
                pred_results = [None] * usable_samples
                raw_outputs = [""] * usable_samples
                final_fail_indices = list(range(usable_samples))
            ...
            for i in range(usable_samples):
                ...
                fallback_reason = "use_llm"

                if not use_llm_this_feature:
                    llm_seq = gnn_seq
                    is_fallback = 1
                    fallback_reason = "feature_disabled"

                else:
                    candidate = pred_results[i]

                    if USE_SAFE_GATE:
                        checked_seq, gate_fallback, gate_reason = safe_gate_llm(
                            llm_seq=candidate,
                            gnn_seq=gnn_seq,
                        ...
                        if gate_fallback:
                            ...
                            if USE_BLEND:
                                llm_seq = blend_llm_gnn(
                                    llm_seq=checked_seq,
                                    gnn_seq=gnn_seq,
                                    feature_id=feature_id
                                )
                                fallback_reason = "use_llm_blend"
                            else:
                                llm_seq = checked_seq
                                fallback_reason = "use_llm_raw"
                            is_fallback = 0
                    else:
                        if candidate is not None:
                            ...
                            if USE_BLEND:
                                llm_seq = blend_llm_gnn(candidate, gnn_seq, feature_id)
                                fallback_reason = "use_llm_blend"
                            else:
                                llm_seq = candidate
                                fallback_reason = "use_llm_raw"
                ...
                timestamp_value = str(station_timestamps[i])
...
with open(GLOBAL_OVERALL_MEAN_PATH, "w", newline="", encoding="utf-8-sig") as f:
    ...
    writer.writerow([
        "SELECTED_STATIONS_ALL_FEATURES",

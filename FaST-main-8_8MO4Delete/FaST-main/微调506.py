...
def build_dataset_fast():
    ...
    stations = list(range(num_stations))  # 👈 只改了这里！训练全部 157 个站点
...
# 检查是否已有生成的数据集，避免重复生成
dataset_path = os.path.join(PROJECT_ROOT, "quick506.json")
...
if generated_dataset:
    ...
    args = TrainingArguments(
        output_dir=os.path.join(PROJECT_ROOT, "results-quick506"),
        num_train_epochs=10,
    ...
    # 断点恢复
    checkpoint_dir = os.path.join(PROJECT_ROOT, "results-quick506")
    ...
    out_path = os.path.join(PROJECT_ROOT, "llama-3-1-8b-highway-finetuned-quick506")

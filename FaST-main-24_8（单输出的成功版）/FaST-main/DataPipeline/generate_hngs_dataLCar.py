...
def load_and_preprocess_data(input_file, output_dir='HNGS_LC'):
    """
    加载并预处理原始数据
...
def generate_meta_data(station_names, output_dir='HNGS_LC'):
    """
    生成站点元数据
...
if __name__ == "__main__":
    ...
    # ========== 第一步：加载和预处理数据 ==========
    # 请修改为你的原始数据文件路径
    input_file = "LittleCar真正名9_10.csv"  # 或 "hunan_highway_traffic_test.xlsx"
    
    try:
        df, station_names = load_and_preprocess_data(input_file, output_dir='HNGS_LC')
    ...
    # ========== 第二步：生成元数据 ==========
    meta_df = generate_meta_data(station_names, output_dir='HNGS_LC')
